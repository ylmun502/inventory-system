...
public class FxUiUtils {
   ...
   *
   * <p>This method is useful to prevent repeated clicks or interactions while an action is being
   * executed. All buttons are re-enabled after the operation completes, regardless of success or
   * failure. <strong>Note:</strong> This runs the operation on the current thread (usually the
   * JavaFX Application Thread), so it is not suitable for long-running tasks.
   *
